...
# DATA_FILES=("sift-128-euclidean-shuffle.hdf5")
DATA_FILES=("gist-960-euclidean-shuffle.hdf5")
...
declare -A DATA_TO_COLUMNS=(
    ...
    # ["sift-128-euclidean-shuffle.hdf5"]="32 64"      
     ["sift-128-euclidean-shuffle.hdf5"]="64"             
...
PHASE1_TOPKS=(100 200 300 400 500 600 700 800 900 1000)
# REDUCE_GROUP_SIZES=(64 128 256 512)
REDUCE_GROUP_SIZES=(128 256)
# 0 = use quant , 1 = use exact
RERANK_METHOD=(0 1)

# Fixed search parameters
PHASE2_TOPK=100
NUM_WORKERS=4
BATCH_NUM_PER_WORKER=256
RERANK_THREAD_POOL_SIZE=16
    ...
    for col_num in "${col_num_array[@]}"; do
        ...
        for qbs in "${QUERY_BATCH_SIZES[@]}"; do
            for rgs in "${REDUCE_GROUP_SIZES[@]}"; do
                for method in "${RERANK_METHOD[@]}"; do

                    # --- Group Header ---
                    # Write the header for this specific group (index_tag, qbs, rgs, rerank_method)
                    # to the single output file *before* iterating through phase1_topks.
                    log "--- Preparing group: index=$index_tag qbs=$qbs rgs=$rgs rerank=$method ---"
                    echo "(index_tag=${index_tag}, query_batch_size=${qbs}, reduce_group_size=${rgs}, rerank_method=${method})" >> "$SEARCH_RESULTS_FILE"
                    ...
                    for p1tk in "${PHASE1_TOPKS[@]}"; do
                        ...
                        log "Executing search: p1tk=$p1tk (Group: index=$index_tag qbs=$qbs rgs=$rgs rerank=$method)"
                        ...
                        if [ $exit_status -ne 0 ]; then
                            log "ERROR: Search command failed with exit status $exit_status for p1tk=$p1tk in group (index=$index_tag qbs=$qbs rgs=$rgs rerank=$method)."
                        ...
                        if [[ -z "$qps" || -z "$recall" ]]; then
                            log "ERROR: Could not parse QPS or Recall from output for p1tk=$p1tk in group (index=$index_tag qbs=$qbs rgs=$rgs rerank=$method)."
                            log "Output was:"
                            echo "$output"
                ...
                log "--- Finished group: index=$index_tag qbs=$qbs rgs=$rgs rerank=$method---"
